...
use memvid_rs::{quick_chat_with_config, chat_with_memory_config, Result};
use std::path::Path;
...
async fn main() -> Result<()> {
    ...
    println!("  Model: {}", model);
    println!("  API Key: {}", if api_key.is_empty() { "Not required" } else { "Set" });
    ...
    match quick_chat_with_config(
        ...
        Err(e) => {
            ...
            println!("4. For other APIs, set OLLAMA_BASE_URL and OLLAMA_MODEL environment variables");
            return Ok(());
        }
    }
