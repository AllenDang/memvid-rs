...
impl QrDecoder {
    ...
    pub fn decode_with_preprocessing(&self, image: &DynamicImage) -> Result<DecodeResult> {
        ...
        Err(MemvidError::QrCode("Failed to decode QR code with all strategies".to_string()))
    }
    ...
    fn decode_with_scaling(&self, image: &DynamicImage) -> Result<DecodeResult> {
        ...
        Err(MemvidError::QrCode("Failed to decode with scaling".to_string()))
    }
...
mod tests {
    ...
    fn create_test_qr_image(text: &str) -> DynamicImage {
        ...
        let resized = frame.image.resize(200, 200, image::imageops::FilterType::Nearest);
        resized
    }
    ...
    fn test_compressed_decode() {
        ...
        let encoder = QrEncoder::new(config);
        let test_text = "This is a longer text that should be compressed and then decompressed correctly.";
    ...
    fn test_batch_decode() {
        ...
        let images: Vec<DynamicImage> = texts.iter()
